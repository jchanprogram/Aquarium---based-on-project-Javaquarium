...
void Aquarium::reproduce()
{
    ...
    else
    {
        if(listOfPoisson[0]->checkSameSpecies(*listOfPoisson[0], *listOfPoisson[1]) && !listOfPoisson[0]->checkSameSexe(*listOfPoisson[0], *listOfPoisson[1]))
        {
            ...
            if(espece=="Merou")
            {
                listOfPoisson.push_back(new Merou(m_nomPoisson,m_sexe,m_age, "Carnivore",espece));
            }
            else if(espece=="Thon")
            {
                listOfPoisson.push_back(new Thon(m_nomPoisson,m_sexe,m_age, "Carnivore",espece));
            }
            else if(espece=="Poisson_Clown")
            {
                listOfPoisson.push_back(new Poisson_Clown(m_nomPoisson, m_sexe, m_age,"Carnivore",espece));
            }
            else if(espece=="Sole")
            {
                listOfPoisson.push_back(new Sole(m_nomPoisson,m_sexe,m_age,"Herbivore",espece));
            }
            else if(espece=="Bar")
            {
                listOfPoisson.push_back(new Bar(m_nomPoisson,m_sexe,m_age, "Herbivore",espece));
            }
            else if(espece=="Carpe")
            {
                listOfPoisson.push_back(new Carpe(m_nomPoisson,m_sexe,m_age, "Herbivore",espece));
            }
        }
        else
        {
            std::cout<<listOfPoisson[0]->getNom()<<" a essaye de fourniquer "<<listOfPoisson[1]->getNom()<<std::endl;
        }
    }
    listOfPoisson[0]->setHaveEatThisTurn(true);
}
...
/*** On en traite pas le cas d'indice 0 dans la boucle pour éviter des pb ***/
    for(unsigned int i = 1; i<listOfPoisson.size();i++)
    {
        ...
        if(listOfPoisson[i]->getEspece()=="Bar" || listOfPoisson[i]->getEspece()=="Merou")
        {
            /******/
            //hermaphroditeWithAge(*listOfPoisson[i]);
        }
    ...
    if(listOfPoisson[0]->getEspece()=="Bar" || listOfPoisson[0]->getEspece()=="Merou")
    {
        /******/
        //hermaphroditeWithAge(*listOfPoisson[0]);
    }
